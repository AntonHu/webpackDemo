(function ($) {
    ...
    $.showTip = function (title, txt, callback, type) {
        ...
        var _tip =
            '<div class="_showTip"><div class="iconfont">' +
            icon +
            '</div><div class="tipTitle">' +
            title +
            '</div><p class="tipTxt">' +
            txt +
            '</p></div>';
    ...
    $.showWashLoading = function (txt) {
        ...
        var _washLoading =
            '<div class="loading_dialog _washLoadingDialog"><div class="weui_mask loading_mask"></div><div class="weui_dialog loading_dialog loading_content" style="z-index:100;"><img width="100%" src="' +
            APP_CONFIG.STATIC_URL +
            'wechat/images/autowash/autowash.gif"/><div class="loading_tip">' +
            txt +
            '</div></div>';
    ...
    $.yglAlert = function (title, img, txt, btn, callback) {
        ...
        var _alert =
            '<div class="_alert-mask"><div class="alert"><div class="title">' +
            title +
            '</div>' +
            imgDom +
            '<div class="alert-txt">' +
            txt +
            '</div><button class="alert-btn">' +
            btn +
            '</button></div></div>';
        $('body').append(_alert);
...
(function (window) {
    ...
    function define_library() {
        var APP_TOOLS = {
            ...
            tempShowActive: function (suc, factoryNo) {
                //是否显示年卡购买入口
                var opts = {};
                if (factoryNo) {
                            ...
                            if (source === 'F6') {
                                ...
                                if (source == 'xkz' && miniApp) {
                                    ...
                                    window.location.href =
                                        APP_CONFIG.APP_SERVICE + 'auth/ali/entry?backUrl=' + encodeURIComponent(url);
                                }
                            } else if (client === 'hcz') {
                                //平安好车主
                                this.getHczUuid(
                                ...
                                window.location.href =
                                    APP_CONFIG.APP_SERVICE + 'weixin/channel/entry2?backUrl=' + encodeURIComponent(url);
                            }
                        }.bind(this)
                    );
                } else {
            ...
            addThirdUser: function (data) {
                ...
                if (data.thirdUserId) {
                    // thirdUserId是第三方用户id，对方有提供，需要保存
                    opts.thirdUserId = data.thirdUserId;
                }
                if (data.source) {
                    // 三方来源，与thirdUserId一起传
                ...
                if (data.factoryNo) {
                    // 可以获取到factoryNo的，记录门店factoryNo，没有的默认YGL0000038
            ...
            getOpenId: function () {
                ...
                } else if (this.sourceFrom() === 'JS_Sinopec') {
                    // 江苏石化
            ...
            setPayMethod: function (num) {
                switch (num) {
                    ...
                    case 22:
                        ...
                        break;
                    case 25:
                        return '汽服卡'; //安徽易捷汽服-汽服卡支付
                        break;
                    case 28:
                        return '优惠券'; //安徽易捷汽服-电子券支付
                        break;
                    case 29:
                        return '余额支付'; //台州章鱼出行余额支付
                        break;
                    case 30:
                        return '优惠券'; //台州章鱼出行优惠券支付
                        break;
                    case 31:
                        return '支付宝'; //app支付宝--直接
                        break;
                    case 32:
                        return '微信'; //app微信支付--直接
                        break;
                    case 33:
                        return '优惠券'; //浙石化洗车券
                        break;
            ...
            getServiceName: function (code, waxingActive, canUse) {
                var subTip = '';
                if (waxingActive && canUse == '1') {
                    //优惠券可用，且开通1/10元打蜡精洗活动
                    subTip = '（活动期间可用于【打蜡精洗】模式）';
                }
                switch (code) {
                    case 'XN000200':
                        return '限【极速快洗】模式使用' + subTip;
                        break;
                    case 'XN000201':
                        return '限【标准普洗】模式使用' + subTip;
                        break;
                    case 'XN000202':
                        return '限【打蜡精洗】模式使用';
                        break;
                    case 'XN000200,XN000201':
                        return '限【极速快洗】【标准普洗】模式使用' + subTip;
                        break;
                    case 'XN000200,XN000202':
                        return '限【极速快洗】【打蜡精洗】模式使用';
                        break;
                    case 'XN000201,XN000202':
                        return '限【标准普洗】【打蜡精洗】模式使用';
                        break;
                }
                return '';
            },
            ...
            getQiaoPaiServiceName: function (code, waxingActive, canUse) {
                ...
                switch (code) {
                    case 'XN000200':
                        ...
                        break;
                    case 'XN000201':
                        return '限【打蜡精洗】模式使用' + subTip;
                        break;
                    case 'XN000202':
                        return '限【炫亮精洗】模式使用';
                        break;
                    case 'XN000200,XN000201':
                        return '限【标准普洗】【打蜡精洗】模式使用' + subTip;
                        break;
                    case 'XN000200,XN000202':
                        return '限【标准普洗】【炫亮精洗】模式使用';
                        break;
                    case 'XN000201,XN000202':
                        return '限【打蜡精洗】【炫亮精洗】模式使用';
                        break;
                }
                return '';
            },
            ...
            verifyServiceUsed: function (ticketCode, modelCode, waxingActive) {
                //waxingActive是否开启10元打蜡精洗活动
                var isUsed = true;
                if (
                    ticketCode &&
                    modelCode &&
                    ticketCode != 'XN000138' &&
                    modelCode != 'XN000138' &&
                    (!waxingActive || modelCode != 'XN000202')
                ) {
            ...
            checkAutoNo: function (no) {
                ...
                if (
                    /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{1}[A-Z]{1}[0-9A-Z]{5}$/.test(no)
                ) {
                    result = autoType['1'];
                    result.autoWord = no.substr(0, 2);
                    result.autoStr = no.substr(2);
                } else if (
                    /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{1}[A-Z]{1}[DF]{1}[0-9A-HJ-Z]{1}[0-9]{4}$/.test(
                        no
                    )
                ) {
                    result = autoType['2'];
                    ...
                    /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{1}[A-Z]{1}[0-9]{5}[DF]{1}$/.test(
                        no
                    )
                ) {
                    result = autoType['3'];
                    ...
                    /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{1}[A-Z]{1}[0-9A-Z]{4}[警]$/.test(
                        no
                    )
                ) {
                    result = autoType['4'];
                    ...
                    /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼]{1}[A-Z]{1}[0-9A-Z]{4}[学]$/.test(
                        no
                    )
                ) {
                    result = autoType['5'];
                    ...
                    /^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼领]{1}[A-Z]{1}[0-9A-Z]{4}[A-Z0-9领挂港澳]$/.test(
                        no
                    )
                ) {
                    result = autoType['999'];
            ...
            isRest: function (data) {
                ...
                if (data.f_work_status) {
                    ...
                    for (var i = 0; i < restInfo.length; i++) {
                        var dateEffective = false,
                            timeEffective = false,
                            restInfoItem = restInfo[i],
                        ...
                        if (restInfoItem.startDate && restInfoItem.startTime) {
                            restInfoItem.startDate =
                                restInfoItem.startDate.split(' ')[0] + ' ' + restInfoItem.startTime;
                        }
                        if (restInfoItem.endDate && restInfoItem.endTime) {
                            if (startTime * 1 > endTime * 1) {
                                //第二天恢复运营的,截止日期要增加一天
                                var newEndDate = new Date(
                                    new Date(restInfoItem.endDate).setDate(new Date(restInfoItem.endDate).getDate() + 1)
                                );
                                restInfoItem.endDate =
                                    tools.getDateFormat(newEndDate, 'Y-M-D') + ' ' + restInfoItem.endTime;
                            }
                        }
                        if (
                            (restInfoItem.startDate &&
                                restInfoItem.endDate &&
                                new Date().getTime() > new Date(restInfoItem.startDate).getTime() &&
                                new Date().getTime() < new Date(restInfoItem.endDate).getTime()) ||
                            (restInfoItem.startDate &&
                                !restInfoItem.endDate &&
                                new Date().getTime() > new Date(restInfoItem.startDate).getTime()) ||
                            (!restInfoItem.startDate &&
                        ...
                        if (
                            ...
                            dateEffective &&
                            timeEffective &&
                            ((restInfoItem.invalidWeek && !new RegExp(week).test(restInfoItem.invalidWeek)) ||
                                !restInfoItem.invalidWeek)
                        ) {
                            workStatusInfo.workStatus = 'rest';
                            workStatusInfo.homeTip = restInfoItem.innerTip
                                ? restInfoItem.innerTip
                                : restInfoItem.homeTip;
            ...
            downloadAPP: function () {
                ...
                if (this.isWechatClient() == 'wx') {
                    ...
                    window.location.href =
                        'itms-apps://itunes.apple.com/cn/app/%E9%A9%BF%E5%85%AC%E9%87%8C%E6%B4%97%E8%BD%A6/id1279764765?mt=8';
                } else {
    ...
    function define_library() {
        var GPS = {
            ...
            gcj_encrypt: function (wgsLat, wgsLon) {
                if (this.outOfChina(wgsLat, wgsLon)) return { latitude: wgsLat, longitude: wgsLon };

                var d = this.delta(wgsLat, wgsLon);
                return {
                    latitude: parseFloat(wgsLat) + parseFloat(d.lat),
                    longitude: parseFloat(wgsLon) + parseFloat(d.lon)
                };
            },

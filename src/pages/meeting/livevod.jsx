            ...
            success: function (res) {
                if (res.status == 1 && res.data) {
                    ...
                    var orderStatus = res.data.f_status,
                        curSerTime = res.data.f_eshop_flow_no
                            ? this.state.serviceTime[res.data.f_eshop_flow_no]
                            : 8 * 60 * 1000,
            ...
            success: function (res) {
                if (res.status == 1 && res.data) {
                    ...
                    if (
                        !(
                            res.data.f_status === 0 ||
                            res.data.f_status === 15 ||
                            res.data.f_status === 25 ||
                            res.data.f_status >= 40
                        )
                    ) {
                        this.orderStatusTimer = setTimeout(
                            function () {
                                this.findStatusByOrderId();
                            }.bind(this),
                            5000
        ...
        return (
            <div id="page-meeting-livevod">
                ...
                <div className="container">
                            ...
                            {messageList.length === 0 && (
                                <div className="empty-item">
                                    <img
                                        ...
                                        src={APP_CONFIG.STATIC_URL + 'wechat/images/active/meeting/empty_image.png'}
                                    />
                                        ...
                                        return (
                                            <div className="message-content">
                                                <div className="message-time">
                                                    {item.dateTime ? tools.getDateFormat(item.dateTime, 'H:I') : ''}
                                                </div>
                                                <div className="message-item">
                                                    {data.content && data.imgUrl && !ifVideo(data.id) && (
                                                        <div className="message-item-mix">
                                                            <img className="message-item-mix-image" src={data.imgUrl} />
                                                            <div className="message-item-mix-text">{data.content}</div>
                                                        </div>
                                                    )}
                                                    {data.imgUrl && !data.content && (
                                                    ...
                                                    {data.content && data.imgUrl && ifVideo(data.id) && (
                                                        <div
                                                            className="message-item-video"
                                                            onClick={_this.goToVideo.bind(_this, data.id)}
                                                        >
                                                            <div className="message-item-video-left">
                                                                <img
                                                                    className="message-item-video-image"
                                                                    src={data.imgUrl}
                                                                />
                                                            </div>
                                                            <div className="message-item-video-right">
                                                                <div className="message-item-video-text">
                                                                    {data.name}
                                                                </div>
                                                                <div className="message-item-video-btn">
                                                                    {'查看演示 >'}
                                                                </div>
                                                            </div>
                                                        </div>
                    ...
                    {st.serviceOrderInfo.f_status == 30 && (
                        <div
                            style={{
                                position: 'fixed',
                                bottom: '60px',
...
ReactDOM.render(<Livevod />, document.getElementById('app'));

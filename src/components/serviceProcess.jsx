...
var ServiceProcessMini = React.createClass({
    ...
    componentWillReceiveProps: function(props){
        if(props.orderEntity){
            if(props.orderEntity.f_status == 0|| props.orderEntity.f_status == 15|| props.orderEntity.f_status == 25){
            ...
            if(props.orderEntity&& props.orderEntity.f_order_id&& !this.state.orderEntity.f_order_id){  //订单信息第一次加载执行
                var curSerTime = props.orderEntity.f_eshop_flow_no? this.state.serviceTime[props.orderEntity.f_eshop_flow_no]: 8*60*1000;
                this.setState({
    ...
    calProcessStatus:function(process){
        ...
        this.loadProcessTimer = setTimeout(function () {
    ...
    showStopTip:function(){     //（显示弹窗）该方法写到父组件中
    ...
    stopMachine:function(){
        ...
        $.ajax({
            ...
            success:function(res){
                if(res.status == 1){
                    //自动会去判断订单是否成功
                    window.location.href = APP_CONFIG.WECHAT2_APP_URL+ 'autowash/reset.html?factoryId='+factoryId+'&orderId='+orderId+'&type=stop';
                }else{
                    $.hideWashLoading();
                    $.msgTip('洗车机关闭失败，请重试')
                }
    ...
    render: function () {
        ...
        return (
            <div className={this.props.fixed? "min-service-process-box min-service-process-box-fixed": "min-service-process-box"}>
                <span className="min-service-status">
                    【{APP_TOOLS.serviceName(orderEntity.f_order_type)}】
                    {minServiceTitle}
                </span>
            ...
            <div className="js_dialog">
                ...
                <div className="weui_dialog stop_dialog" style={{width:"90%",left:"5%"}}>
                    <div className="weui_dialog_hd">
                        ...
                        <img style={{width:"60px",margin:"15px auto"}} src={APP_CONFIG.STATIC_URL+ 'wechat/images/autowash/warn.png'} />
                    </div>
                    <div className="weui_dialog_bd" style={{lineHeight:"19px",textAlign:"justify",color:"#333333"}}>停止洗车机，强行结束当前洗车服务，如需再次服务需要重新扫码支付，请谨慎操作。</div>
                    <div className="weui_dialog_ft" style={{padding:"4px 0px"}}>
                        <a onClick={this.cancel} className="weui_dialog_btn weui_dialog_btn_default" style={{color:"#333333",borderRight:"1px solid #D5D5D6"}}>取消</a>
                        <a onClick={this.stopMachine} className="weui_dialog_btn weui_dialog_btn_primary" style={{color:"#FF4C00"}}>结束洗车</a>
                    </div>
                </div>
            </div>
        )

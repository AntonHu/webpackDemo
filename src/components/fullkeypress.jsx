    ...
    render: function (){
        ...
        for(var i=0;i<this.str.length;i++){
            ...
            for(var j=0;j<this.str[i].length;j++){
                ddNode.push(
                    <dd onTouchEnd={ this.wordClickHandle.bind(this) }>{ this.str[i][j] }</dd>
                )
            }
            kpStrNode.push(
                <dl>{ ddNode }</dl>
        ...
        for(var i=0;i<this.num.length;i++){
            kpNumNode.push(
                <dd onTouchEnd={ this.wordClickHandle.bind(this) }>{ this.num[i] }</dd>
            )
        }
        ...
        return(
            <div className="key-press full" onClick={ this.hide } style={{ 'display': this.state.dis }}>
                <div className="press-box" onClick={ this.stopPropagation }>
                    <div className={ this.props.entry === '-----' ? "interim-check active" : "interim-check" } onClick={ this.interimClickHandle }>
                        <em></em>
                        <span>我是临时车牌</span>
                    </div>
...
var AutoNoInput = React.createClass({
    ...
    switchAutoNoType: function(type){
        ...
        if(type == 2){
            ...
            if(document.querySelector('#auto-hand-input-'+this.index)){
                document.querySelector('#auto-hand-input-'+this.index).value = this.props.autoWord+this.props.autoStr
            }
            this.props.hidePress()
        }else {
            if(document.querySelector('#auto-hand-input-'+this.index)){
            ...
            <div className="autono-input-wrapper">
                {this.props.noSpecial? null: 
                    <div className="autono-input-switch">
                        <span className={st.autoNoType == 1? "autono-label-item active": "autono-label-item"} onClick={this.switchAutoNoType.bind(this, 1)}>普通车牌</span>
                        <span className={st.autoNoType == 2? "autono-label-item active": "autono-label-item"} onClick={this.switchAutoNoType.bind(this, 2)}>特殊车牌</span>
                    </div>
                }
                <div className={isGreen? "carsno green": "carsno"} style={{display:st.autoNoType == 1? 'block': 'none'}}>
                    <div className="f-choosebtn" onClick={ this.props.showKeyPress }>
                        <span className="f-result">{ this.props.autoWord } <i></i></span>
                    </div>
                    <div className="end-string">
                        <input readOnly="true" value={ autoStr } onClick={ this.props.showFullKeyPress } className={ autoStr ? 'active' : '' } type="url" placeholder="输入车牌号"/>
                    </div>
                </div>
                {this.props.noSpecial? null: 
                    <div className="carsno" style={{display:st.autoNoType == 2? 'block': 'none'}}>
                        <input className="auto-hand-input" id={"auto-hand-input-"+this.index} type="text" placeholder="如：沪A0006警" onChange={this.inputAutoNo} />
                    </div>
                }
            </div>
        )

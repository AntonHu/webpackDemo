var TicketUseFactorysList = React.createClass({
    render: function () {
        ...
        var factoryScreenData = this.props.factoryScreenData,
            formatScreenData = [];
        ...
        var list = formatScreenData.map(
            function (item, i) {
                ...
                return (
                    <div style={styles.subtitle_2}>
                        {item.cityName}：<span style={{ color: '#999' }}>{itemList}</span>
                    </div>
                );
            }.bind(this)
        );
        ...
        if (this.props.type) {
            ...
            if (ticketScreenType == '13') {
                //限站点
                subtitle1 = '该优惠券只限驿公里以下站点使用。';
                subtitle2 = '支持使用的站点：';
            }
        }
        if (this.props.shareInviteTicket) {
            //分享券站点列表文字

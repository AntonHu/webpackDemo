...
var ImageItem = React.createClass({
    ...
    render: function () {
        ...
        for(var i=0; i<imgList.length; i++){
            if(imgList[i]){
                var imgBigItem = /(http:\/\/|https:\/\/)/.test(imgList[i])? imgList[i]: APP_CONFIG.IMG_URL +'Big/'+ imgList[i];
        ...
        var styles = {
            ...
            closeIcon: {width: '16px', height: '16px', position: 'absolute', top:'-8px', right:'-3px', zIndex:'2', display:this.props.showClose&& this.props.showClose=='true'? 'inline-block': 'none'}
        };
        return (
            <div style={styles.imgWrap}>
                <div style={styles.imgBox}>
                    <img style={styles.closeIcon} src={APP_CONFIG.STATIC_URL + 'wechat/images/common/close.png'} onClick={this.deleteImg.bind(this, this.props.index)}/>
                    <img style={styles.imgStyle} src={newImg} onClick={this.previewHandle.bind(this, imgBigUrl, imgBigList)}/>
                </div>
            </div>
        )
    }

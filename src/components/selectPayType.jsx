var SelectPayType = React.createClass({
    getInitialState: function(){
        return{
            configPayType: [        //配置实例
                {title: '年卡支付', icon: this.STATIC_IMGURL+"pay-vip-icon.png", type: '9', value: '剩余0次', tip:'购买年卡享9折起'},
    ...
    cancel: function(){
        ...
        $('#mask-pop').animate({'opacity': '0'}, 200, function(){$('#select-pay-type').css('display', 'none');});
    ...
    render: function(){
        ...
        var payTypeList = this.props.configPayType.map(function(item, i){
            var topTitleStyle = {
                    lineHeight:'46px',
                    fontSize:'17px',
                    fontWeight:'300'
                ...
                <li style={liStyle} onClick={this.chooseType.bind(this, item)}>
                    <div style={styles.liContentStyle}>
                        <div style={{height:'100%', padding:'0px 40px 0px 72px', position: 'relative'}}>
                            ...
                            <span className={this.props.payType == item.type? "select-icon select":"select-icon"}></span>
                        </div>
                    </div>
                </li>
            )
        ...
        return(
            <div id="select-pay-type" style={styles.wrapperStyle} onClick={this.cancel}>
                ...
                <div id="btn-list" style={styles.listBox} onClick={this.cancelBubble}>
                    <div style={styles.titleStyle}><img style={styles.closeImg} src="../../images/common/close_1.png" onClick={this.cancel}/>选择支付方式</div>
